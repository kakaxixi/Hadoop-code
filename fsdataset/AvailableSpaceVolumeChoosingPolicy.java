...
private double ioUsage;
  ...
  public synchronized void setConf(Configuration conf) {
    ...
    LOG.info("Available space volume choosing policy initialized: --wanglai-available-setconf" +
        DFS_DATANODE_AVAILABLE_SPACE_VOLUME_CHOOSING_POLICY_BALANCED_SPACE_THRESHOLD_KEY +
	...
	try {
		...
		LOG.info("--wanglaiavailable--cpuusage:"+this.cpuUsage);
		mpstatReader.readLine();
		mpstatReader.readLine();
		mpstatLine = mpstatReader.readLine();
		if (mpstatLine == null) {
			...
			this.ioUsage = Double
					.parseDouble(mpstatChunkedLine[mpstatChunkedLine.length - 1]);
			LOG.info("--wanglaiavailable--iousage:"+this.ioUsage);
		}
	} catch (Exception e) {
		try {
			throw e;
    ...
    if (volumesWithSpaces.areAllVolumesWithinFreeSpaceThreshold()) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug("All volumes are within the configured free space balance " +
            "threshold. Selecting --wanglaichoosevolume" + volume + " for write of block size " +
            replicaSize);
      }
      return volume;
    } else {
      ...
      if (mostAvailableAmongLowVolumes < replicaSize ||
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug("Volumes are imbalanced. Selecting " + volume +
              " from high available space volumes for write of block size --wanglai-available-choosevolume"
              + replicaSize);
        }
      } else {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug("Volumes are imbalanced. Selecting " + volume +
              " from low available space volumes for write of block size --wanglai-available-choosevolume"
              + replicaSize);
        }
      }
      return volume;
